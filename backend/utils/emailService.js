...
async function sendPasswordResetEmail(to, otp, name = 'User') {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: 'MORYA MEDICAL <admin@moryamedical.shop>',
      to: [to],
      subject: 'Password Reset Request - Morya Medical',
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
          <h2 style="color: #1f2937;">Morya Medical</h2>
          <p>Hi ${name},</p>
          ...
          <p style="font-size: 24px; font-weight: bold; letter-spacing: 4px; color: #2563eb;">${otp}</p>
          <p style="color: #6b7280; font-size: 14px;">This code expires in 15 minutes. If you didn't request a password reset, you can safely ignore this email. Your password will remain unchanged.</p>
          <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;" />
          <p style="color: #9ca3af; font-size: 12px;">Morya Medical - Community Engagement Project</p>
        </div>
      `,
